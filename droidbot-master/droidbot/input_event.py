...
class EventLog(object):
    ...
    def start(self):
        ...
        if "intent='am start" in self.event_str :
            app_package = self.event_str.split(" ")[2].split("/")[0]
            if not self.app.isStart :
                self.grant_all_perm(app_package)
            self.hook_application_start(app_package)
            self.app.isStart = True
            time.sleep(5)
        else :
            self.device.send_event(self.event)
        
    def grant_all_perm(self, app_package) :
        self.device.logger.info(self.app.app_path)
        task = subprocess.Popen("aapt d permissions " + self.app.app_path, shell=True, stdout=subprocess.PIPE)
        task_out = task.stdout.read().splitlines()
        for line in task_out :
            perm = line.decode('utf-8').replace("uses-permission: name=", "").replace("'", "")
            self.device.logger.info("perm : " + perm)
            if not app_package in perm :
                subprocess.run("adb shell pm grant " + app_package + " " + perm, capture_output=False)
                
        self.device.logger.info("========== grant all permission ==========")
